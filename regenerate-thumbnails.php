...
Plugin Name:  Regenerate Thumbnails + External (S3)
Plugin URI:   http://www.viper007bond.com/
Description:  Regen thumbnails + externally hosted images
Version:      2.2.6
Author:       Impression
...
<?php
	...
	public function ajax_process_image() {
		...
		if ( false === $fullsizepath || ! file_exists( $fullsizepath ) ){
			...
			$save_path_parts = pathinfo($fullsizepath);
			$path = wp_upload_dir()['path'].'/'.$save_path_parts['filename'].'.'.$save_path_parts['extension'];
			if($fpc = file_put_contents($path, fopen($fullsizepath, 'r'))){
				$fullsizepath = $path;
			}else{
				$this->die_json_error_msg( $image->ID, sprintf( __( 'The originally uploaded image file cannot be found at %s', 'regenerate-thumbnails' ), '<code>' . esc_html( $fullsizepath ) . ' '. $path .' '. $fpc . '</code>' ) );
			}
